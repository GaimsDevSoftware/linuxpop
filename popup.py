...
class PopupWindow:
    ...
    def show_for(
        ...
        if force_all:
            ...
            # Sort the same way for_content_type does so pinned plugins and
            # plugin_order are honoured in the expanded view too.
            try:
                from settings import get_settings as _gs
                _s = _gs()
                order = list(_s.get("plugin_order") or [])
                pinned = list(_s.get("pinned_plugins") or [])
            except Exception:
                order, pinned = [], []
            order_index = {n: i for i, n in enumerate(order)}
            pinned_index = {n: i for i, n in enumerate(pinned)}
